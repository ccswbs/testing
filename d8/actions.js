...
const LoginEndpoint = "/user/login" + format;
const TokenEndpoint = "/rest/session/token";
const NodeEndpoint = "/entity/node" + format;
...
async function getLoginHeaders(user, pass) {
	...
	var b64Cred = new Buffer(user + ':' + pass).toString('base64');

	headers['Authorization'] = 'Basic ' + b64Cred;
...
module.exports = {
	...
	CreateNode:async function(type, node_data) {
		...
		var nid = -1;
		await request({
			uri:Env.baseURL + NodeEndpoint,
			headers:headers,
			method:"POST",
			json:data
		}).then(function(body) {
			nid = body.nid[0].value;
		}).catch(function(err) {
			if(err) console.log(err.message);
		});
